...
export class InvadersComponent implements OnInit {
  ...
  spawnRowOfEnemies(yOffSet: number) {
    for (let i = 0; i < this.ctx.canvas.height / 20; i++) {
      if (Math.random() < 0.01) {
        this.enemies.push(new Enemy(this.ctx, i * 20 + (i * 10), 10 + yOffSet, 10, 10, 'green', 500));
      } else {
        this.enemies.push(new Enemy(this.ctx, i * 20 + (i * 10), 10 + yOffSet, 10, 10, 'blue', 100));
      }
    }
  }
  ...
  gameLoop = () => {
    if (this.$stopped.value) {
      this.firestore.collection("users").doc(this.username).set({
      name: this.username,
      score: this.score
    });
      return;
